...
Видим, что маршрут в СПб, Триаду может проходить через Москву. 
...
Маршрутизаторы, граничащие с Москвой, не получают от ее автономной системы транзитные маршруты.
...
В принципе, при текущей топологии не должно быть транзитного трафика. Причина - R18 говорит, что знает маршрут до Москвы через AS520.  Этот апдейт приходит к единственным соседям в зоне AS520, они его отбрасывают, т.к. видят свою AS в пришедшем апдейте.
...
Маршрут по умолчанию получен. Но почему есть и более подробные маршруты??? clear bgp ipv4 unicast * не исправил проблему. Нужно выполнить фильтрацию. Причем лучше делать на R14, чтобы быть уверенным, что не прилетит куча маршрутов.

Выполним фильтрацию на стороне R14:

```
en
conf t
ip prefix-list PL_IN_N10 permit 0.0.0.0/0
ip prefix-list PL_IN_N10 deny 0.0.0.0/0 le 32


ipv6 prefix-list PL_IN_N10 permit ::/0
ipv6 prefix-list PL_IN_N10 deny ::/0 le 128

router bgp 1001
address-family ipv4 unicast
neighbor 20.255.255.10 prefix-list PL_IN_N10 in

address-family ipv6 unicast
neighbor 2001:db8:255:255:8::10 prefix-list PL_IN_N10 in
...
![](screenshots/2021-06-14-16-12-07-image.png)

![](screenshots/2021-06-14-16-30-28-image.png)

Из Киторна приходит только маршрут по умолчанию.
...
Задача выполнена. Лучше делать фильрацию на входе в Москве, но задача сформированна про Ламас.
