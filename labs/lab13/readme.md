...
Соседства не устанавливается, не приходят hello-пакеты, на R15 команда debug eigrp packets включает дебаг, но ничего не приходит.

!!! Напоминание про работу EIGRP. Соседство будет установлено только с тех интерфейсов, которые попадают в команду network. Если нет ни одной команды, то соседство не будет устанавливаться ни с одного интерфейса. И сообщать маршрутизатор будет только о тех маршрутах, которые попадают в команду network.
...
Это происходит из-за анонса белого адреса туннеля с R18. С прошлых лабораторных работ этот анонс был незамечен, да и вообще не нужен. Отключаем.
...
Но на R15 появился маршрут D EX до сети 20.78.0.0/24. Это сеть, которая анонсируется с R18 путем добавления статического маршрута. В EIGRP была задана команда редистрибуции статики, что тоже нужно убрать. Но возникает вопрос, почему R15 выбрал в качестве маршрута именно маршрут через туннель, а не через белые сети провайдеров, ведь AD BGP много меньше EIGRP?

Разбираемся:

![](screenshots/2021-06-26-14-28-43-image.png)

![](screenshots/2021-06-26-14-35-25-image.png)

R15 получает маршрут по iBGP, от соседа R14, от провайдера он маршрут не получил. Поэтому все логично - AD iBGP 200, перебивается метрикой EIGRP 170.

Но почему R15 предпочел маршрут через iBGP и R14, а не eBGP и R21? Потому что на предыдущих лабораторных работах выполнена настройка - R15 получает только маршрут по умолчанию, без подробностей. 

Итак, на R18 отключаем редистрибуцию в EIGRP. R18:
