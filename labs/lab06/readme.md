...
![](screenshots/2021-05-13-23-27-17-image.png)
...
1. Добавить прямое соединение между маршрутизаторами. Потребуется замена маршрутизаторов либо установка плат расширения для увеличения количества портов. Для виртуальной лаборатории абсолютно не сложно. Поэтому вариант отложим=).
...
3. Расширить нулевую зону до R12-15. 
   ...
   ![](screenshots/2021-05-13-23-42-30-image.png)
...
4. Заменить R12-13 на L3-коммутаторы. Это позволит избавиться от BVI. В реальности будет дороже 2,3го вариантов (возможно и 1го тоже), в лабораторной будет сложнее 1,2,3го вариантов. Но позволит избавиться от BVI и отработать настройку L3-коммутаторов. Кроме того, если мы захотим снова поднять границу зоны 0 наверх (исключить из нее SW12-13), это будет достаточно просто сделать, т.к. R13-14 все равно смогут друг друга видеть через коммутаторы. Остановимся на нем. Измененная схема (т.е. мы поставим SW12-13 вместо маршрутизаторов, а затем расширим границу зоны 0 вниз до SW12-13):
   
   ![](screenshots/2021-05-13-23-37-15-image.png)
...
![](screenshots/2021-05-12-15-16-41-image.png)
...
## <a name="head135"></a> Маршрутизатор R19 находится в зоне 101 и получает только маршрут по умолчанию
...
![](screenshots/2021-05-12-20-38-09-image.png)
