...
Остается выполнить фильтрацию маршутов. Она выполняется на R16. Задаем префикс-лист и используем его в исходящем направлении на интерфейсе для фильтрации. В ipv4 минимальная длина префикса 1.
...
Нисправляем - добавляем в префикс-лист разрешающее правило для маршрута 0.0.0.0/0.0.0.0.
...
Маршруты до всех подсетей пропали. Возможно, префикс-лист порезал и маршрут по умолчанию. Но такой маршрут и ранее не передавался, что отмечено выше.
...
Эти подсети  - 10.78.10.0/24,10.78.40.0/24,10.78.80.0/24. Адресной пространство за маршрутизаторами запланировано так, что за ними расположены сети 10.78.0.0/16. Поэтому будем отдавать именно такой саммарный адрес.
