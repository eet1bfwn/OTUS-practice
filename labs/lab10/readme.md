...
Выход в другие автономные системы стал возможен и через соседа по зоне - он стал следующим хопом. При этом в таблице нет маршрута до сети Москвы 10.177.0.0/16, которая анонсируется соседом (см. R14) - причина в том, что сосед находится в том же VLAN???? Хотя на R15 маршрут через самого себя отсутствует, а идет через соседа.

![](screenshots/2021-06-01-22-40-14-image.png)



Проблема в R15 - он не анонсирует сеть 10.177.0.0/16. Для проверки можем вообще погасить R14, тогда маршрутизаторы из других автономных систем не узнают о Москве.
...
??? Все же что значит suppressed??? И что будет с таким сценарием - сосед транслирует несуммаризованный маршрут по BPG, этот маршут принимается и вытесняет OSPF, который более оптимальный. Проверим: отключим суммаризацию. Но маршрут по BGP не попал в таблицу. Объяснение простое - AD iBGP 200, в отличие от eBPG c 20. Все логично.
...
Здесь еще вопрос??? На R14-15 заданы дефолтные маршруты, чтобы передавать его по OSPF. Как можно избавиться от дефолта? Ручной записью:
...
Прописываем дефолт в Null, чтобы соседи по EIGRP все равно получали маршрут по умолчанию. Однако в другой лабораторной работе маршрут до определенной сети и полученный по EIGRP, отправленный в Null записью ip route, не транслировался соседям. Странно.
