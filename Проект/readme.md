...
- Назначить каждому серверу серый IP, и при входе в автономную систему пакета до сервера менять адрес назначения с белого на серый.
  - Но в этом случае при обращении к серверу из серой сети пакет пойдет сперва на пограничный маршрутизатор, там произойдет трансляция адреса назначения, пакет направится к серверу.
